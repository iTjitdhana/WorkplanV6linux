@echo off
echo ========================================
echo 🐳 Pull และ Run WorkplanV6 จาก Docker Hub
echo ========================================
...
docker pull %username%/%image_name%-frontend:%version%
if %errorlevel% neq 0 (
...
docker pull %username%/%image_name%-backend:%version%
if %errorlevel% neq 0 (
...
docker run -d ^
  ...
  %username%/%image_name%-backend:%version%
...
docker run -d ^
  ...
  --restart unless-stopped ^
  %username%/%image_name%-frontend:%version%
