...
# Script สำหรับ push Docker images ขึ้น Docker Hub ด้วย custom tag
echo "🚀 เริ่มต้น push Docker images ขึ้น Docker Hub ด้วย custom tag..."

# ตั้งค่า Docker Hub credentials
DOCKER_USERNAME="itjitdhana"
IMAGE_NAME="workplanv6.4"

# ตรวจสอบว่ามี tag parameter หรือไม่
if [ -z "$1" ]; then
    echo "❌ กรุณาระบุ tag"
    echo "💡 ตัวอย่าง: ./docker-push-tag.sh v1.0.0"
    echo "💡 ตัวอย่าง: ./docker-push-tag.sh latest"
    exit 1
fi
...
# Tag images
echo "🏷️ Tag images ด้วย version: $VERSION..."
