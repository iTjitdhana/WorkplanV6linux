...
export default function MedicalAppointmentDashboard() {
  ...
  // ฟังก์ชันโหลดข้อมูลห้องผลิต
  const loadRooms = async () => {
    try {
      const response = await fetch('/api/production-rooms');
      const data = await response.json();
      if (data.success && data.data) {
        setRooms(data.data || []);
      }
  ...
  const getRoomName = (roomCodeOrId: string | number) => {
    ...
    return room ? room.room_name : (typeof roomCodeOrId === 'number' ? roomCodeOrId.toString() : roomCodeOrId);
  };

  // ฟังก์ชันจัดการสถานะงานตาม syncModeEnabled
  const getJobStatus = (item: any) => {
    // งานปกติที่มีใน Table work_plans (ไม่ใช่ draft) ควรเป็น "พิมพ์แล้ว"
    if (!item.isDraft && item.workflow_status_id) {
      return "พิมพ์แล้ว";
    }
    ...
    // สำหรับงานใน Table work_plans ที่มี workflow_status_id = 2 (บันทึกเสร็จสิ้น)
        ...
        <div className="w-full px-2 sm:px-4 md:px-6 lg:px-8">
          <div className="flex justify-between items-center h-12 sm:h-14 md:h-16 lg:h-18">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 md:space-x-3 lg:space-x-4 flex-shrink-0">
              ...
              <span className="hidden lg:block text-xs md:text-sm text-green-100">ระบบจัดการข้อมูล</span>
                                      ...
                                      >
                                        ...
                                        <div className="mt-1 flex items-center space-x-1">
                                            ...
                                            className={`inline-block px-1 sm:px-1.5 py-0.5 rounded text-xs ${
                                              getJobStatus(item) === "พิมพ์แล้ว"
                                                ...
                                                : getJobStatus(item) === "บันทึกเสร็จสิ้น" || getJobStatus(item) === "บันทึกสำเร็จ"
                                                  ? "bg-green-100 text-green-700"
                                                  : getJobStatus(item) === "รอดำเนินการ"
                                                    ? "bg-gray-100 text-gray-700"
                                                    : getJobStatus(item) === "กำลังดำเนินการ"
                                                      ? "bg-blue-100 text-blue-700"
                                                      : getJobStatus(item) === "เสร็จสิ้น"
                                                        ? "bg-green-100 text-green-700"
                                                        : getJobStatus(item) === "งานผลิตถูกยกเลิก"
                                          ...
                                          {shouldShowSpecialJobLabel(item) && (
                                            <span className="inline-block px-1 sm:px-1.5 py-0.5 rounded text-xs bg-yellow-100 text-yellow-800 border border-yellow-300">
                                              งานพิเศษ
                                            </span>
                                          )}
                                        </div>
