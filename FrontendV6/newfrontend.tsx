...
export default function MedicalAppointmentDashboard() {
  ...
  const staffImages: { [key: string]: string } = {
    จรัญ: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/%E0%B8%88%E0%B8%A3%E0%B8%B1%E0%B8%8D_4-GFrLyijyGYklkZcNwkdvR8EMU5hBJm.jpeg",
    จิ๋ว: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/%E0%B8%88%E0%B8%B4%E0%B9%8B%E0%B8%A7-jylak0613JyGbDHW86C8eFZohEZj5g.png",
  ...
  const getWeekDates = (date: Date) => {
    ...
    // Only add Monday to Saturday (6 days instead of 7)
    for (let i = 0; i < 6; i++) {
  ...
  // ฟังก์ชันสำหรับจัดลำดับงานใหม่ในแต่ละวัน
  const getProductionWithDailySequence = (productions: any[]) => {
    // จัดกลุ่มตามวัน
    const groupedByDate = productions.reduce(
      (acc, item) => {
        if (!acc[item.date]) {
          acc[item.date] = []
        }
  ...
  const weekRange = `${formatFullDate(weekDates[0])} - ${formatFullDate(weekDates[5])}`
  ...
  const getWeekProduction = () => {
    ...
    return getProductionWithDailySequence(filteredData)
  }

  // Get production data for selected day
  const getSelectedDayProduction = () => {
    const filteredData = productionData.filter((item) => item.date === selectedDate)
    return getProductionWithDailySequence(filteredData)
  }
                      ...
                      <div className="text-center">
                        ...
                        <p className={`text-gray-600 mt-1 ${isFormCollapsed ? "text-sm" : "text-xs"}`}>
                          ...
                          {weekDates.reduce((total, date) => {
                            ...
                            return total + dayProduction.length
                          }, 0)}{" "}
                          งาน
                        </p>
                      </div>
                    ...
                    <div className="overflow-x-auto">
                      <div className="min-w-full">
                        ...
                        <div className="grid grid-cols-6 gap-1">
                          {weekDates.map((date, index) => {
                            const dateStr = date.toISOString().split("T")[0]
                            const dayProduction = getProductionWithDailySequence(
                              productionData.filter((item) => item.date === dateStr),
                            )
                                  ...
                                  <div className="space-y-2 flex-1">
                                    {dayProduction.map((item) => (
                                      ...
                                      >
                                        ...
                                        >
                                          <span className="underline">งานที่ {item.dailySequence} :</span> {item.title}
                                        </div>
